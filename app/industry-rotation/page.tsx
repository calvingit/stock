...
const INDUSTRY_ETFS = '512880,513100,515230,512480,512690,515790,515050,516970,512890,512200,515120,512670,512660,512760,512010,512610,515030,513130,518880';

export default function IndustryRotationPage() {
  const [params, setParams] = useState({ short_ma: 5, long_ma: 15, signal_ma: 50, stop_loss: 7, n_stocks: 2 });
  ...
  useEffect(() => {
    run({ codes: INDUSTRY_ETFS, fast_ma: params.short_ma, mid_ma: params.long_ma, slow_ma: params.signal_ma, trail_stop: params.stop_loss / 100, hard_stop: 0, begin: '2022-07-01', end: '2026-06-30', mode: 'etf', initial_capital: 1000000, volume_confirm: 0, pause_after_losses: 0 });
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">申万行业 ETF 轮动 + CSI300 择时 + 移动止损 · 持仓 {params.n_stocks} 只</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 p-4 rounded-lg border border-border bg-card">
          ...
          <Slider label="持仓数" value={params.n_stocks} min={1} max={5} step={1} onChange={(v) => setParams(p => ({ ...p, n_stocks: v }))} />
          <button onClick={() => run({ codes: INDUSTRY_ETFS, fast_ma: params.short_ma, mid_ma: params.long_ma, slow_ma: params.signal_ma, trail_stop: params.stop_loss / 100, hard_stop: 0, begin: '2022-07-01', end: '2026-06-30', mode: 'etf', initial_capital: 1000000, volume_confirm: 0, pause_after_losses: 0 })} disabled={loading}
          ...
          {item ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <StatCard label="胜率" value={`${item.win_rate.toFixed(0)}%`} color="text-cyan-400" />
              </div>
